...
export default ({ navigation }) => {
   ...
   const updateProfile = async (route) => {
      switch (route) {
         case 'name':
            if (newName !== null) {
               const result = await Axios.default.post('https://falhub.com/api/updateProfile?place=name', { name: newName, u_id: user._id, token: await AsyncStorage.getItem('token'), device: await DeviceInfo.getAndroidId() })
         ...
         case 'mail':
            if (newMail !== null) {
               const resultMail = await Axios.default.post('https://falhub.com/api/updateProfile?place=mail', { mail: newMail, u_id: user._id, token: await AsyncStorage.getItem('token'), device: await DeviceInfo.getAndroidId() })
      ...
      try {
         ...
         if (granted === PermissionsAndroid.RESULTS.GRANTED) {
            ...
            if (result.didCancel) {
               ...
               const verify = await Axios.default.post('https://falhub.com/api/updatePhoto?verify=true', { token: await AsyncStorage.getItem('token'), device: await DeviceInfo.getAndroidId(), u_id: user._id })
               if (verify.data.success) {
                  ...
                  const photoResult = await Axios.default.post('https://falhub.com/api/updatePhoto?savePhoto=true', data, {
                     headers: {
